...
@RestController @RequestMapping("/shop/order") public class MallOrderController extends BaseController {
 ...
 @Value("${mall.payment.manual-confirm-enabled:false}") private boolean manualPaymentEnabled;
 ...
 @PreAuthorize("@ss.hasPermi('shop:order:pay')") @Log(title="商城订单",businessType=BusinessType.UPDATE) @PutMapping("/{orderId}/pay-success") public AjaxResult paySuccess(@PathVariable Long orderId,@RequestBody MallOrder order){if(!manualPaymentEnabled)throw new ServiceException("人工支付确认未启用");return toAjax(orderService.paySuccess(orderId,order.getPayChannel(),order.getChannelTradeNo(),order.getPaidAmount(),"ADMIN",getUsername()));}
 @PreAuthorize("@ss.hasPermi('shop:order:cancel')") @Log(title="商城订单",businessType=BusinessType.UPDATE) @PutMapping("/{orderId}/cancel") public AjaxResult cancel(@PathVariable Long orderId,@RequestBody MallOrder order){return toAjax(orderService.cancelOrder(orderId,order.getCancelReason(),"ADMIN",getUsername()));}
 @PreAuthorize("@ss.hasPermi('shop:order:ship')") @Log(title="商城订单",businessType=BusinessType.UPDATE) @PutMapping("/{orderId}/ship") public AjaxResult ship(@PathVariable Long orderId,@RequestBody MallOrder order){return toAjax(orderService.shipOrder(orderId,order.getLogisticsCompany(),order.getLogisticsNo(),"ADMIN",getUsername()));}
 @PreAuthorize("@ss.hasPermi('shop:order:ship')") @Log(title="商城订单",businessType=BusinessType.UPDATE) @PutMapping("/{orderId}/complete") public AjaxResult complete(@PathVariable Long orderId){return toAjax(orderService.completeOrder(orderId,"ADMIN",getUsername()));}
}
