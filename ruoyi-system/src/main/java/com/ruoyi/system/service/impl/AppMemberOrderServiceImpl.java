...
@Service public class AppMemberOrderServiceImpl implements IAppMemberOrderService {
 ...
 public List<MallOrder> list(Long memberId,String status){MallOrder query=new MallOrder();query.setMemberId(memberId);query.setOrderStatus(status);List<MallOrder> orders=orderMapper.selectOrderList(query);if(orders.isEmpty())return orders;List<Long> orderIds=new ArrayList<Long>();for(MallOrder order:orders)orderIds.add(order.getOrderId());Map<Long,List<MallOrderItem>> itemsByOrder=new HashMap<Long,List<MallOrderItem>>();for(MallOrderItem item:orderMapper.selectOrderItemsByOrderIds(orderIds))itemsByOrder.computeIfAbsent(item.getOrderId(),key->new ArrayList<MallOrderItem>()).add(item);for(MallOrder order:orders)order.setItems(itemsByOrder.getOrDefault(order.getOrderId(),new ArrayList<MallOrderItem>()));return orders;}
 ...
 public int cancel(Long memberId,String orderNo,String reason){MallOrder order=orderMapper.selectOrderByMemberNo(memberId,orderNo);if(order==null)throw new ServiceException("订单不存在");return orderService.cancelOrder(order.getOrderId(),StringUtils.isBlank(reason)?"会员取消订单":reason,"MEMBER",String.valueOf(memberId));}
 public int confirmReceipt(Long memberId,String orderNo){MallOrder order=orderMapper.selectOrderByMemberNo(memberId,orderNo);if(order==null)throw new ServiceException("订单不存在");return orderService.completeOrder(order.getOrderId(),"MEMBER",String.valueOf(memberId));}
