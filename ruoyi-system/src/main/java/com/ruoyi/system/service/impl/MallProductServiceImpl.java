...
public class MallProductServiceImpl implements IMallProductService
{
    ...
    @Override @Transactional(rollbackFor = Exception.class)
    public int updateProduct(MallProduct product) { MallProduct existing = selectProductById(product.getProductId()); if (existing == null) throw new ServiceException("商品不存在或已删除"); validateSkuChanges(existing, product); validateProduct(product); productMapper.updateProduct(product); saveSkus(product); deleteOmittedSkus(existing, product); return 1; }
    ...
    private void validateSkuChanges(MallProduct existing, MallProduct submitted) { Set<Long> existingIds = new HashSet<Long>(); for (MallProductSku sku : existing.getSkus()) existingIds.add(sku.getSkuId()); Set<Long> submittedIds = new HashSet<Long>(); for (MallProductSku sku : submitted.getSkus() == null ? Collections.<MallProductSku>emptyList() : submitted.getSkus()) { if (sku.getSkuId() != null && !existingIds.contains(sku.getSkuId())) throw new ServiceException("SKU不存在或不属于当前商品"); if (sku.getSkuId() != null) submittedIds.add(sku.getSkuId()); } for (MallProductSku sku : existing.getSkus()) if (!submittedIds.contains(sku.getSkuId()) && sku.getLockedStock() != null && sku.getLockedStock() > 0) throw new ServiceException("SKU存在锁定库存，不能删除"); }
    private void deleteOmittedSkus(MallProduct existing, MallProduct submitted) { Set<Long> submittedIds = new HashSet<Long>(); for (MallProductSku sku : submitted.getSkus() == null ? Collections.<MallProductSku>emptyList() : submitted.getSkus()) if (sku.getSkuId() != null) submittedIds.add(sku.getSkuId()); for (MallProductSku sku : existing.getSkus()) if (!submittedIds.contains(sku.getSkuId()) && productMapper.softDeleteSku(existing.getProductId(), sku.getSkuId()) == 0) throw new ServiceException("SKU存在锁定库存，不能删除"); }
