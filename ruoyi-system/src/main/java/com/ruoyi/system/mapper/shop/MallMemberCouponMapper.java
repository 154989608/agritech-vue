...
public interface MallMemberCouponMapper { MallCoupon selectCouponForReceive(Long couponId); List<MallCoupon> selectAvailableCoupons(); List<Long> selectReceivedCouponIds(@Param("memberId") Long memberId,@Param("couponIds") List<Long> couponIds); int countByCouponAndMember(@Param("couponId") Long couponId,@Param("memberId") Long memberId); int countAvailableByMember(Long memberId); int increaseReceived(Long couponId); int insertMemberCoupon(MallMemberCoupon coupon); List<MallMemberCoupon> selectMemberCouponList(MallMemberCoupon coupon); MallMemberCoupon selectAvailableById(@Param("memberCouponId") Long memberCouponId,@Param("memberId") Long memberId); int lockCoupon(@Param("memberCouponId") Long memberCouponId,@Param("memberId") Long memberId,@Param("orderId") Long orderId,@Param("orderAmount") Long orderAmount); int unlockCoupon(Long orderId); int useCoupon(Long orderId); int expireCoupons(); }
