<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="orders">
      ...
      <el-table-column label="操作" width="220" fixed="right"><template #default="scope"><el-button link type="primary" v-hasPermi="['shop:order:query']" @click="showDetail(scope.row)">详情</el-button><el-button v-if="scope.row.orderStatus === 'PENDING_PAYMENT'" link type="danger" v-hasPermi="['shop:order:cancel']" @click="doCancel(scope.row)">取消</el-button><el-button v-if="scope.row.orderStatus === 'PENDING_SHIPMENT'" link type="primary" v-hasPermi="['shop:order:ship']" @click="openShip(scope.row)">发货</el-button></template></el-table-column>
    </el-table>
...
import { cancelOrder, getOrder, listOrder, shipOrder } from '@/api/shop/order'
