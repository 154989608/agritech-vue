  ...
  <div class="screen-container">
    ...
    <div class="screen-header">
      <div class="header-left">
        <span class="header-sub">STATION HEAT EFFICIENCY</span>
      </div>
      <div class="header-center">
        <div class="title-decoration left"></div>
        <h1 class="screen-title">城市公交站点热力效能分析</h1>
...
const formatAxisValue = (value) => value >= 10000 ? `${(value / 10000).toFixed(1)}万` : Number(value).toLocaleString()
...
const darkTheme = { backgroundColor: 'transparent', textStyle: { color: '#a0b4d0' } }

const initStationHeatmap = () => {
  if (!stationHeatmap.value) return
  const chart = echarts.init(stationHeatmap.value, 'dark')
  ...
  chart.setOption({
    ...darkTheme,
    tooltip: {
      position: 'top', backgroundColor: 'rgba(10,20,50,0.9)', borderColor: '#00d4ff', textStyle: { color: '#e0e8ff' },
    ...
    xAxis: {
      ...
      axisTick: { show: false },
      axisLabel: { color: '#6a8caa', fontSize: 11, interval: 0, margin: 12 },
    ...
    yAxis: {
      ...
      axisLine: { lineStyle: { color: '#1a3a5c' } },
      axisTick: { show: false },
      axisLabel: { color: '#6a8caa', fontSize: 11, margin: 10 },
      splitArea: { show: false }
    },
    visualMap: {
      min: 0, max: 100, calculable: true, orient: 'horizontal', left: 'center', bottom: 0,
      textStyle: { color: '#6a8caa', fontSize: 11 },
...
const initTopStationsChart = () => {
  if (!topStationsChart.value) return
  const chart = echarts.init(topStationsChart.value, 'dark')
  const stations = ['二环东路', '解放路', '山大南路', '泉城路', '历山路', '燕子山', '洪家楼', '北园大街', '经十路', '商职学院']
  const values = [8600, 9800, 11500, 12300, 13600, 15200, 16800, 17200, 17800, 18500]
  chart.setOption({
    ...darkTheme,
    ...
    yAxis: {
      ...
      axisLine: { show: false },
      axisTick: { show: false },
      axisLabel: { color: '#6a8caa', fontSize: 11 },
    },
    series: [{
      type: 'bar', data: values, barWidth: 12,
      itemStyle: {
        color: new echarts.graphic.LinearGradient(0, 0, 1, 0, [
          { offset: 0, color: '#0050ff' }, { offset: 1, color: '#00d4ff' }
        ]),
...
const initEfficiencyChart = () => {
  ...
  const chart = echarts.init(efficiencyChart.value, 'dark')
  chart.setOption({
    ...darkTheme,
...
const initPeriodChart = () => {
  ...
  const chart = echarts.init(periodChart.value, 'dark')
  chart.setOption({
    ...darkTheme,
    ...
    xAxis: {
      ...
      data: ['00:00','04:00','06:00','08:00','10:00','12:00','14:00','16:00','18:00','20:00','22:00'],
      axisLine: { lineStyle: { color: '#1a3a5c' } },
      axisTick: { show: false },
      axisLabel: { color: '#6a8caa', fontSize: 10, margin: 10 },
    ...
    series: [{
      data: [800, 450, 1200, 6800, 4200, 3800, 3500, 3800, 6200, 5100, 2800],
      type: 'line', smooth: true,
      lineStyle: { color: '#ff9f43', width: 2 },
      itemStyle: { color: '#ff9f43' },
      areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
        { offset: 0, color: 'rgba(255,159,67,0.4)' }, { offset: 1, color: 'rgba(255,159,67,0.02)' }
      ])}
    }]
  })
  charts.push(chart)
...
const initZoneTypeChart = () => {
  if (!zoneTypeChart.value) return
  const chart = echarts.init(zoneTypeChart.value, 'dark')
  chart.setOption({
    ...darkTheme,
...
.screen-container {
  ...
  .screen-header {
    ...
    &::before, &::after {
      content: ''; position: absolute; bottom: -1px; width: 80px; height: 2px;
      background: #00d4ff; box-shadow: 0 0 10px #00d4ff;
    }
    &::before { left: 0; } &::after { right: 0; }
    .header-sub { font-size: 11px; letter-spacing: 3px; color: rgba(0,212,255,0.5); }
    .header-center {
      display: flex; align-items: center; gap: 16px;
      .title-decoration { width: 60px; height: 1px; background: linear-gradient(90deg, transparent, #00d4ff); &.right { transform: scaleX(-1); } }
      .screen-title { margin: 0; font-size: 22px; font-weight: bold; color: #e0f4ff; letter-spacing: 2px; text-shadow: 0 0 20px rgba(0,212,255,0.6); }
    }
    .header-time { font-size: 13px; color: rgba(0,212,255,0.7); font-family: 'Courier New', monospace; letter-spacing: 1px; }
  }
  ...
  .stats-row {
    ...
    .stat-card {
      ...
      border: 1px solid rgba(0,212,255,0.2); border-radius: 6px; padding: 12px 16px;
      display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;
      &::before { content: ''; position: absolute; top: 0; left: 0; width: 3px; height: 100%; background: linear-gradient(180deg, #00d4ff, transparent); }
      .stat-icon-wrap { width: 38px; height: 38px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; .stat-icon { font-size: 22px; } }
      .stat-body { flex: 1;
  ...
  .chart-panel {
    ...
    border: 1px solid rgba(0,212,255,0.15); border-radius: 6px; overflow: hidden; position: relative;
    &::after { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, rgba(0,212,255,0.5), transparent); }
    .panel-header {
      display: flex; align-items: center; justify-content: space-between; padding: 10px 16px; border-bottom: 1px solid rgba(0,212,255,0.1);
  ...
  .data-table {
    ...
    th {
      font-size: 12px; color: #6a8caa; font-weight: normal; padding: 8px 12px; text-align: left;
