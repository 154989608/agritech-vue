  ...
  <div class="screen-container">
    ...
    <div class="screen-header">
      <div class="header-left">
        <span class="header-sub">CAPACITY DIAGNOSIS & DECISION</span>
      </div>
      <div class="header-center">
        <div class="title-decoration left"></div>
        <h1 class="screen-title">城市公交运力诊断决策分析</h1>
      ...
      <div class="table-panel">
        ...
        <div class="table-wrap">
          <table class="data-table">
            <thead>
              <tr>
                <th>线路</th><th>配车数</th><th>间隔(分)</th><th>满载率</th><th>准点率</th><th>状态</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(row, i) in lineData" :key="i">
                ...
                <td>
                  <span class="val-tag" :class="getLoadClass(row.loadFactor)">{{ row.loadFactor }}%</span>
                </td>
                <td>
                ...
                <td>
                  <span class="status-tag" :class="row.status === '正常' ? 'ok' : 'warn'">{{ row.status }}</span>
                </td>
              </tr>
            </tbody>
          </table>
...
const initCapacityChart = () => {
  ...
  const chart = echarts.init(capacityChart.value, 'dark')
  chart.setOption({
    ...darkTheme,
    ...
    xAxis: {
      type: 'category', data: ['K239路', '692路', 'K148路', '101路', 'K56路', '102路', 'K7路'],
    ...
    yAxis: {
      type: 'value', name: '人次/小时', nameTextStyle: { color: '#6a8caa' },
      axisLine: { show: false },
      axisTick: { show: false },
...
const initCancelRateChart = () => {
  if (!cancelRateChart.value) return
  const chart = echarts.init(cancelRateChart.value, 'dark')
  ...
  chart.setOption({
    ...
    xAxis: {
      ...
      axisLine: { lineStyle: { color: '#1a3a5c' } },
      axisTick: { show: false },
      axisLabel: { color: '#6a8caa', fontSize: 10, interval: 4, margin: 10 },
      splitLine: { show: false }
    },
    yAxis: {
      type: 'value',
      ...
      { name: '作废率', type: 'bar', data, barWidth: '58%', itemStyle: { borderRadius: [2, 2, 0, 0] } },
      {
        ...
        areaStyle: {
          color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
            { offset: 0, color: 'rgba(255,159,67,0.16)' },
            { offset: 1, color: 'rgba(255,159,67,0.01)' }
          ])
        }
      }
    ]
...
const initVehicleChart = () => {
  ...
  const chart = echarts.init(vehicleChart.value, 'dark')
  chart.setOption({
    ...darkTheme,
...
const initDiagnosisChart = () => {
  if (!diagnosisChart.value) return
  const chart = echarts.init(diagnosisChart.value, 'dark')
  chart.setOption({
    ...darkTheme,
    tooltip: { trigger: 'axis', backgroundColor: 'rgba(10,20,50,0.9)', borderColor: '#00d4ff', textStyle: { color: '#e0e8ff' } },
    ...
    yAxis: {
      ...
      axisLine: { show: false },
      axisTick: { show: false },
      axisLabel: { color: '#6a8caa', formatter: v => `${v}%`, margin: 10 },
      splitLine: { lineStyle: { color: '#0d2240', type: 'dashed' } }
    },
    series: [
      {
        name: '准点率', type: 'line', smooth: true, data: [92, 94, 91, 93, 89, 95, 96],
      ...
      {
        name: '满载率', type: 'line', smooth: true, data: [78, 82, 85, 80, 88, 75, 72],
...
const initCardTypeChart = () => {
  if (!cardTypeChart.value) return
  const chart = echarts.init(cardTypeChart.value, 'dark')
  chart.setOption({
    ...darkTheme,
    ...
    xAxis: {
      type: 'category', data: ['K239路', '692路', 'K148路', '101路', 'K56路'],
      axisLine: { lineStyle: { color: '#1a3a5c' } },
      axisTick: { show: false },
      axisLabel: { color: '#6a8caa', fontSize: 11, margin: 10 },
      splitLine: { show: false }
...
.screen-container {
  ...
  .screen-header {
    ...
    border-bottom: 1px solid rgba(0,212,255,0.3); padding: 0 20px; position: relative;
    &::before, &::after { content: ''; position: absolute; bottom: -1px; width: 80px; height: 2px; background: #00d4ff; box-shadow: 0 0 10px #00d4ff; }
