...
@Component public class AppMemberAuthService {
    public static final String MEMBER_TOKEN_KEY=CacheConstants.APP_MEMBER_TOKEN_KEY;
    ...
    public void logout(String token){if(StringUtils.isEmpty(token))return;AppMemberSession session=getSession(token);redisCache.deleteObject(MEMBER_TOKEN_KEY+token);if(session!=null)redisCache.redisTemplate.opsForSet().remove(CacheConstants.APP_MEMBER_TOKENS_KEY+session.getMemberId(),token);}
    // ponytail: the set uses the latest session TTL; use Redis expiry events only if stale token IDs become material.
    private void storeSession(String token,AppMemberSession session){redisCache.setCacheObject(MEMBER_TOKEN_KEY+token,session,sessionMinutes,TimeUnit.MINUTES);String indexKey=CacheConstants.APP_MEMBER_TOKENS_KEY+session.getMemberId();redisCache.redisTemplate.opsForSet().add(indexKey,token);redisCache.expire(indexKey,sessionMinutes,TimeUnit.MINUTES);}
